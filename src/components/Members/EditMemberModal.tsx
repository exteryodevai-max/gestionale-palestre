...
import { X, User, Mail, Phone, Calendar, FileText, Save, AlertCircle } from 'lucide-react'
...
export function EditMemberModal({ isOpen, onClose, onMemberUpdated, member }: EditMemberModalProps) {
  const { user } = useAuth()
  ...
  useEffect(() => {
    if (member) {
      setFormData({
        ...
        certificato_valido_fino: member.certificato_valido_fino || '',
        stato: member.stato || 'attivo',
        note: member.note || ''
      })
    }
  }, [member])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('✅ Iscritto aggiornato:', data)
      ...
      setErrors({ submit: error.message || 'Errore durante l\'aggiornamento dell\'iscritto' })
    } finally {
      setLoading(false)
    }
  }
