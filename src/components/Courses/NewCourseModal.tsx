...
import { supabase, Area, User as UserType } from '../../lib/supabase'
...
export function NewCourseModal({ isOpen, onClose, onCourseCreated, areas, trainers }: NewCourseModalProps) {
  const { user } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('✅ Nuovo corso creato:', data)
      ...
      setErrors({ submit: error.message || 'Errore durante la creazione del corso' })
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Nuovo Corso</h2>
              <p className="text-sm text-gray-600">Crea un nuovo corso per la tua palestra</p>
            </div>
          </div>
          <button
            onClick={onClose}
