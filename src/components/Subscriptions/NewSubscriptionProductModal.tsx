...
import { supabase, DurationUnitType } from '../../lib/supabase'
import { useAuth } from '../../hooks/useAuth'

interface NewSubscriptionProductModalProps {
  isOpen: boolean
  onClose: () => void
...
export function NewSubscriptionProductModal({ isOpen, onClose, onProductCreated }: NewSubscriptionProductModalProps) {
  const { user } = useAuth()
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('✅ Nuovo prodotto abbonamento creato:', data)
      ...
      console.error('❌ Errore creazione prodotto abbonamento:', error)
      setErrors({ submit: error.message || 'Errore durante la creazione del prodotto abbonamento' })
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900">Nuovo Prodotto Abbonamento</h2>
              <p className="text-sm text-gray-600">Definisci un nuovo tipo di abbonamento per la tua palestra</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Descrizione opzionale del prodotto abbonamento..."
                  />
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="mt-4 p-4 bg-purple-50 rounded-lg border border-purple-200">
                <h4 className="font-medium text-purple-900 mb-2">Anteprima Prodotto</h4>
                <div className="text-sm space-y-1">
                  <div>
                    <span className="text-purple-700 font-medium">Nome:</span>
                    <span className="ml-2 text-purple-900">{formData.name || 'Nome prodotto'}</span>
                  </div>
                  <div>
                    <span className="text-purple-700 font-medium">Prezzo:</span>
                    <span className="ml-2 text-purple-900">€{formData.price.toFixed(2)}</span>
                  </div>
                  <div>
                    <span className="text-purple-700 font-medium">Durata:</span>
                    <span className="ml-2 text-purple-900">{getDurationPreview()}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-6">
              ...
              <div className="flex items-center">
                ...
                <label htmlFor="is_active" className="ml-2 block text-sm text-gray-900">
                  Prodotto attivo (disponibile per la vendita)
                </label>
              </div>
            </div>
          </div>
